...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       NetWMFullscreen, NetActiveWindow, NetWMWindowType, BlockRoundedCorner,
       NetWMWindowTypeDialog, NetClientList, NetClientInfo, NetLast }; /* EWMH atoms */
enum { WMProtocols, WMDelete, WMState, WMTakeFocus, WMLast }; /* default atoms */
enum { ClkTagBar, ClkLtSymbol, ClkStatusText, ClkWinTitle,
       ClkClientWin, ClkRootWin, ClkLast }; /* clicks */
enum { String, Integer, Float }; /* resource types for .Xresources*/
...
static void allowroundedcorner(Client *c);
static void blockroundedcorner(Client *c);
static void smartcorners(void);
...
setup(void)
{
  ...
  netatom[BlockRoundedCorner] = XInternAtom(dpy, "_BLOCK_ROUNDED_CORNER", False);
	/* init cursors */
...
void
allowroundedcorner(Client *c)
{
    unsigned long data = 0;
    c->BLOCK_ROUNDED_CORNER = 0;
    XChangeProperty(dpy, c->win, netatom[BlockRoundedCorner], XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&data, 1);
}

void
blockroundedcorner(Client *c)
{
    unsigned long data = 1;
    c->BLOCK_ROUNDED_CORNER = 1;
    XChangeProperty(dpy, c->win, netatom[BlockRoundedCorner], XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&data, 1);
}
...
smartcorners(void)
{
    Client *c;
    int gaps_enabled = 0;
    // Check if gaps are enabled for current tag
    gaps_enabled = selmon->pertag->enablegaps[selmon->pertag->curtag];
    
    // Apply to all visible clients on current tag
    for (c = selmon->clients; c; c = c->next) {
        if (ISVISIBLE(c)) {
            if (c->isfloating) {
                allowroundedcorner(c);  // Floating windows always get rounded corners
            } else if (!gaps_enabled) {
                blockroundedcorner(c);  // Gaps disabled = block rounded corners
            } else {
                allowroundedcorner(c);  // Gaps enabled = allow rounded corners
            }
        }
    }
}
